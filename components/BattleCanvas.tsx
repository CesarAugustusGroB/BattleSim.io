...
const BattleCanvas: React.FC<BattleCanvasProps> = ({ engine, onCanvasClick }) => {
    ...
    useEffect(() => {
        ...
        const initPixi = async () => {
            ...
            // Pixi 8
            await app.init({
                resizeTo: containerRef.current || undefined, // Auto resize to container
                background: '#101010',
                antialias: true,
                autoStart: false,
            ...
            if (containerRef.current) {
                ...
                // Manual Resize Observer for robustness
                const resizeObserver = new ResizeObserver(() => {
                    if (app.renderer && containerRef.current) {
                        app.resize();
                        // Update globals if needed? 
                        // Note: Constants like CANVAS_WIDTH are imported but actual view is dynamic now.
                        // We might need to update simulation bounds logic if sim relies on constant width.
                    }
                });
            ...
            const renderLoop = () => {
                ...
                if (Math.random() < 0.01) {
                    console.log(`[BattleCanvas] Rendering ${ents.length} entities. App Stage Children: ${app.stage.children.length}`);
                }
                ...
                for (let i = 0; i < ents.length; i++) {
                    ...
                    const radius = engine.UnitComponent.radius[eid];
                    const team = engine.UnitComponent.team[eid];
                    const health = engine.UnitComponent.health[eid];
                    const maxHealth = engine.UnitComponent.maxHealth[eid];
                    const lastHit = engine.UnitComponent.lastHitTime[eid];
                    const px = engine.Position.x[eid];
                    const py = engine.Position.y[eid];
        ...
        return () => {
            ...
            if (appRef.current) {
                ...
                if (appAny._resizeObserver) {
                    appAny._resizeObserver.disconnect();
                }
    ...
    return (
        <div
            ref={containerRef}
            className="w-full h-full block bg-[#101010] cursor-crosshair touch-none min-h-[600px] min-w-[800px]"
